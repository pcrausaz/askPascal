## Bunch of other links

- [RADVISION](http://www.radvision.com/)
- [GatherWorks](http://www.gatherworks.com/)
- [Asterisk](http://www.asterisk.org/)
- [Google](http://www.google.com/)
- [IG Nobel](http://ignobel.com/)
- [Checker Shadow Illusion](http://web.mit.edu/persci/people/adelson/checkershadow_illusion.html)
- [Apple](http://www.apple.com/)
- [mdsn.microsoft.com](http://msdn.microsoft.com/)
- [Bink Windows XP](http://winxp.bink.nu/)
- [3DGamers](http://www.3dgamers.com/)
- [Cool\_Applications\_for\_MacOSX]

## Stats and Counters

- [StatCounter](http://www.statcounter.com/)
- [Net Stats Basic - Web Site Statistics](http://www.nedstatbasic.net/s?tab=2&link=1)